...
- `signals-pattern` — the pub/sub spine; read this first if it's your first Pexip app
- `media-pipeline` — `mediaStream` you pass to `.call()` comes from `@pexip/media`'s `mediaService`
- `reconnect` — how `useOnFailedInfinityRequest` and `NetworkState` cooperate with this flow
- `preflight` — what runs before this skill kicks in
- `browser-close-confirmation` — the `beforeunload` companion to the `pagehide` handler
- `stats-monitoring` — the `onRtcStats` handler shape used in this skill is documented in detail there
- `live-captions` — `onLiveCaptions` events flow through this skill's event handlers
