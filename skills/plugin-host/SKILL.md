...
- `branding-manifest` — `manifest.json` declares plugins, host loads them
- `signals-pattern` — plugin events forward through the same signal hubs
- `participants` — `event:participants` payload comes from `meeting.getParticipants`
- `call-lifecycle` — `event:userInitiatedDisconnect` and conference RPCs route through the meeting service
