...
- `plugin-host` — manifest's `plugins` array drives plugin loading
- `signals-pattern` — `config.subscribe(key, handler)` is how branding-driven config changes propagate
- `preflight` — `bgImageAssets` and `customStepConfig` affect the join flow
