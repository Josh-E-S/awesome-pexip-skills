...
The non-obvious bit. Some filter results depend on others — e.g., the `InMeeting` filter is "Host ∪ Guest", so changing host/guest invalidates `InMeeting`. Webapp3 expresses this as a forward-dependency map and computes the reverse at startup:
...
When a participant's `Transferring` flag changes, all groups that depend on it (`Host`, `Guest`, `Breakout`, `DirectChat`, `FECC`, `External`, `RaisedHand`, `WaitingInLobby`) get their caches invalidated. The DFS-builder in `createReversedDependencyAdjacencyList` even checks for cycles and throws if you make a mistake.

You don't need to touch this map unless you add a new `GroupKey`. If you do — declare its forward dependencies and the rest is automatic.
...
- `signals-pattern` — `infinityClientSignals` is a batched-aware hub
- `call-lifecycle` — `getMe()` and the FECC pid signal state
- `chat` — direct messages need the `DirectChat` group filter
- `breakouts` — every breakout has its own room id with its own participant list
- `fecc` — `GroupKey.FECC` filters who can be remote-controlled
- `plugin-host` — plugins receive `event:participants` events sourced from this util
