...
- `call-lifecycle` — how the InfinityClient signal hubs are wired into the meeting state machine
- `media-pipeline` — how Media signals coordinate track lifecycle with the SDK
- `reconnect` — how `NetworkState` (a behavior signal in `@pexip/media-components`) gates other subscribers
- `participants` — uses the **batched** signal variant for `onParticipantJoined`/`Updated`/`Left`
