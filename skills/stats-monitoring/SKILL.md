...
- `signals-pattern` — `callSignals` is a `createCallSignals` hub
- `media-pipeline` — bandwidth changes interact with these stats (`config.subscribe('bandwidth', …)`)
- `reconnect` — quality-driven adaptive bandwidth is *not* part of webapp3; the SDK handles ICE/quality internally
