---
name: browser-close-confirmation
...
- `branding-manifest` — `enableBrowserCloseConfirmationByDefault` is a brand-level default
- `call-lifecycle` — `pagehide` (different from `beforeunload`) is what tells the server to free the slot
