...
This skill captures the right structure. The deep details live in the parent `media-pipeline` skill (`createMedia` setup) — preflight is mostly about the **UI hooks** that `@pexip/media-components` provides, plus the browser-detection pattern.
...
- `media-pipeline` — the full `createMedia` setup; preflight is a UI layer over it
- `signals-pattern` — `mediaSignals.onMediaChanged` is what drives device-list reactivity
- `call-lifecycle` — what happens when the user clicks "Join"
- `branding-manifest` — `customStepConfig` adds an extra card to this flow
