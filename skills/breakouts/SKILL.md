...
- `participants` — `GroupKey.Breakout*` filters; the per-room participant tracking
- `chat` — chat is cleared on `onBreakoutRefer`
- `call-lifecycle/transfer-flow.md` — the underlying transfer mechanism
- `signals-pattern` — `setBreakoutStepSignal` is a local signal hub
