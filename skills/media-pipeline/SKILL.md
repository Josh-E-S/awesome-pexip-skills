...
- `signals-pattern` — `mediaSignals` is a `createMediaSignals` hub; understand the variants
- `preflight` — uses preview hooks (`createPreviewHook`, etc.) before the main pipeline starts
- `call-lifecycle` — `clientMute` / `muteVideo` API calls coordinate with `mediaSignals.onMediaTrackMuted`
- `reconnect` — `applyConstraints` + bandwidth changes during reconnect
- `branding-manifest` — `applicationConfig.audioProcessing` / `videoProcessing` flags toggle whole subsystems
- `fecc` — PTZ constraints (`pan`/`tilt`/`zoom`) flow through `getDefaultConstraints`
