...
This skill is about the *UI layer*. The actual recovery — calling `restartCall` to revive the peer connection — is handled at the service layer (see `call-lifecycle` skill). The two layers cooperate:
...
- `call-lifecycle` — `onPeerDisconnect` → `restartCall` is the recovery mechanism this skill renders UI for
- `signals-pattern` — `callSignals` is a `createCallSignals` hub from `@pexip/infinity`
- `media-pipeline` — bandwidth changes during reconnect can be triggered by quality monitoring (out of scope here, but related)
