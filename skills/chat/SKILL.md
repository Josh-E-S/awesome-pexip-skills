...
- `signals-pattern` — `onMessage` is on `infinityClientSignals`; `chatMessageSignal` is a local hub
- `call-lifecycle` — chat state survives direct-media transfers (see `transfer-flow.md`)
- `participants` — direct messages need participant uuids; the participants util is the source
