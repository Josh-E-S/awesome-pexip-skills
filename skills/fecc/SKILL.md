...
`SignalState` is webapp3's wrapper around `createSignal({variant: 'behavior'})` plus a getter — it's reactive state that re-renders subscribers when the value changes (see `signals-pattern`).
...
- `participants` — `GroupKey.FECC` filter; `participant.canFecc` flag
- `media-pipeline/video-effects.md` — the PTZ constraints in `getDefaultConstraints`
- `signals-pattern` — `SignalState` is the behavior-signal pattern in disguise
